...
pipeline{
    ...
    stages{
        stage("Checkout"){
            steps{
              git branch:"final" , url:"https://github.com/bankino-microservice/legacyadapter.git",
        ...
        stage('Deploy Configuration') {
            steps {
                sh '''
                # Create application.properties file for Spring Boot
                mkdir -p ./src/main/resources
                cat > ./src/main/resources/application-prod.properties <<EOF

spring.application.name=legacy-adapter-service
server.port=8085
legacy.soap.url=http://localhost:8080/ws
   ...
   stage("Trigger remote Trivy scan") {
    steps {
        script {
            ...
            def imageTag = "${IMAGE_NAME}:legacy${BUILD_NUMBER}"
